...
class UserClass extends React.Component {
  constructor(props) {
    ...
    this.state = {
      userInfo: {
        name: "Dummy Name",
        location: "Default",
        avatar_url: "https:Dummy Picture",
      },
      // count: 0,
    };
  ...
  async componentDidMount() {
    const data = await fetch("https:api.github.com/users/AmitDaware");
    const json = await data.json();

    // as soon a compiler hits this setState "THE UPDATING LIFECYCLE STARTS..." now due to the set state method the state gets updated and then the react start the render method again.
  ...
  render() {
    ...
    return (
      <div className="user">
        <div class="bg-black text-gray-700 relative   flex items-center justify-center overflow-hidden z-50 ">
            ...
            <div class="flex flex-col items-center justify-between lg:flex-row py-16">
              <div class=" relative ">
                ...
                <div class="lg:max-w-xl lg:pr-5 relative z-40">
                  <h1 className="font-extrabold text-4xl  font-mono">{name}</h1>
                  <h2 className="font-sans">{location}</h2>
                  <h2 class="mb-6 max-w-lg text-5xl font-light leading-snug tracking-tight text-g1 sm:text-7xl sm:leading-snug">
                    We make you look
                    <span class="my-1 inline-block border-b-8 border-gray-800 bg-black px-4 font-bold text-g4 animate__animated animate__flash">
                      different
                    </span>
                  </h2>
